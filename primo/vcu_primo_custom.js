...
$(document).ready(function(){
  //the UNIVERSITY variable will be used to put in the special requirements of the 
  //Qatar campus!!!

  var UNIVERSITY = window.location.href.match(/vid=VQA/) ? "QATAR" : "VCU";

  //determining when we are on course reserves view & adding 'help' link
  if (window.location.href.match(/tab=cr/)) {
    $('#exlidHomeContainer').prepend('<a class="reserves-help" title="cr help" href="http://www.library.vcu.edu/research/reserves/">Course Reserves Help</a>');
    $('#exlidResultsContainer').prepend('<a class="reserves-help" title="cr help" href="http://www.library.vcu.edu/research/reserves/">Course Reserves Help</a>');
  }

  $('body').attr("style", "margin-left: auto !important; margin-right: auto !important;");

  //special rule for QATAR campus
  if(UNIVERSITY === "QATAR"){
    $("#exlidSearchTabs li").attr("style", "margin: 0 6px;");
  } 
  
  //ajax call for the VCU "ad words" -- only for VCU campus\
  if(UNIVERSITY === "VCU"){
    var searched_term = $('#search_field').val();
    if(searched_term !== ""){
      $.ajax({
        url:'https://apps.library.vcu.edu/assets/shared/includes/extra_search.php?q=' + encodeURI(searched_term),
        complete: function(data){
          var responseJSON = jQuery.parseJSON(data.responseText);
          if(data.status === 200){
            console.log('success~!!');
            console.log(responseJSON);
            console.log(responseJSON.results.guides.length !== 0);
    
            var db_length = parseInt(responseJSON.results.dbs.SEGMENTS.JAGROOT.RESULT.DOCSET["@TOTALHITS"]);
            if(( db_length !== 0 && !isNaN(db_length) )  || responseJSON.results.guides.length !== 0 ){
              buildSuccessBlock(responseJSON, searched_term);
            }  
          }else{
            console.log('error for extra search: ');
            console.log('response status is ' + data.status );
            console.log('response text is ' + data.statusText);
          }
        }
      });
    }

    //adding VCU branding bar
    var brandingMarkup = "<div id='vcuIdentityWrapper'><header id='vcuIdentity'><h1><a title='Virginia Commonwealth University' target='_blank' href='http://www.vcu.edu'>Virginia Commonwealth University</a></h1><h2><a title='VCU Make it real.'' target='_blank' href='http://www.makeitreal.vcu.edu'>Make it real.</a></h2></header></div>";
    $('body').prepend(brandingMarkup);

  }
  //this where we put together the Ad Words plus block
  function buildSuccessBlock(json, query){
    var htmlText = "<div class='extra-results'>";
    var length = parseInt(json.results.dbs.SEGMENTS.JAGROOT.RESULT.DOCSET["@TOTALHITS"]);
    //console.log(!isNaN(length));
    if(length !== 0 ){
      htmlText += "<p>Related Databases</p>";
      if(length == 1){ htmlText += addEntry(json.results.dbs.SEGMENTS.JAGROOT.RESULT.DOCSET.DOC, 'db'); }
      if(length > 1){  htmlText += addEntry(json.results.dbs.SEGMENTS.JAGROOT.RESULT.DOCSET.DOC[0], 'db')} 
      htmlText += "<a onclick='ga(\"send\", \"event\", \"link\", \"click\", \"" + ((window.location.hostname == 'vcu-primo.hosted.exlibrisgroup.com') ? 'dev-adwords-database-link' : 'prod-adwords-database-link' )+ "\", {\"page\": \"https://apps.library.vcu.edu/dblist/search?q=" + encodeURIComponent(query) +"\"});' href='https://apps.library.vcu.edu/dblist/search?q=" + encodeURIComponent(query) + "'>See more database results &raquo;</a>";
    }

    if(json.results.guides.length !== 0){
      htmlText += "<p>Related Research Guides</p>";
      htmlText += addEntry(json.results.guides[0], 'guide');
      htmlText += "<a onclick='ga(\"send\", \"event\", \"link\", \"click\", \"" + ((window.location.hostname == 'vcu-primo.hosted.exlibrisgroup.com') ? 'dev-adwords-libguides-link' : 'prod-adwords-libguides-link' )+ "\", {\"page\": \"http://guides.library.vcu.edu/srch.php?q=" + encodeURIComponent(query) +"\"});' href='http://guides.library.vcu.edu/srch.php?q=" + encodeURIComponent(query) + "'>See more research guides &raquo;</a>";
    }

    htmlText + "</div>";

    //we are about to show the expanded results, so let's end a Goolge Analytics event
    $('#resultsListNoId').prepend(htmlText);
        ga("send", {'hitType': 'event', 'eventCategory': 'adwords_shown', 'eventAction':'show', 'nonInteraction': true});

  }

  //this is where we store the template for each entry in the Ad Words plus
  function addEntry(entry, type){
      var htmlText = '';
      console.log(entry);
      if(type === "db"){
        htmlText += "<strong><a onclick='ga(\"send\", \"event\", \"link\", \"click\", \"" + ((window.location.hostname == 'vcu-primo.hosted.exlibrisgroup.com') ? 'dev-adwords-database-link' : 'prod-adwords-database-link' )+ "\", {\"page\": \"" + entry.PrimoNMBib.record.display.identifier +"\"});' href='" + entry.PrimoNMBib.record.display.identifier + "'>" + entry.PrimoNMBib.record.display.title + "</a></strong><span> &mdash; " + entry.PrimoNMBib.record.display.description +"</span></br>";
      }

      if(type === "guide"){
         htmlText += "<strong><a onclick='ga(\"send\", \"event\", \"link\", \"click\", \"" + ((window.location.hostname == 'vcu-primo.hosted.exlibrisgroup.com') ? 'dev-adwords-libguides-link' : 'prod-adwords-libguides-link' )+ "\", {\"page\": \"http://guides.library.vcu.edu/c.php?g=" + entry.id +"\"});' href='http://guides.library.vcu.edu/c.php?g=" + entry.id + "'>" + entry.name + "</a></strong></br>";
      }
  ...
  var bookplateStyle='</br>';
  $('h3.EXLResultFourthLine a').before(bookplateStyle);
...
// 'Check Status' Illiad Link
  ...
  var fourth = ["P", "Q", "R", "S", "T", "U", "V", "W", "X", "Y", "Z"];

   $('.EXLResultStatusAvailable').each(function( i ) {
    var a = $(this).find('.EXLAvailabilityLibraryName');
    var b = $(this).find('.EXLAvailabilityCollectionName');
    var c = $(this).find('.EXLAvailabilityCallNumber');

      if ((a.text().trim() == 'Cabell Library') && (b.text().trim() == 'General Collection')) {
        var x = c.text().trim().charAt(1);

      if (jQuery.inArray( x, fourth ) !== -1) {
        console.log(c.text().trim().charAt(1));
        $(this).after('</br><div class="floor-button"><strong> 4th Floor</strong></div>');
      }

      if (jQuery.inArray( x, third ) !== -1) {
        console.log(c.text().trim().charAt(1));
        $(this).after('</br><div class="floor-button"><strong> 3rd Floor</strong></div>');
      }

      if (jQuery.inArray( x, first ) !== -1) {
        console.log(c.text().trim().charAt(1));
        $(this).after('</br><div class="floor-button"><strong> 1st Floor</strong></div>');
      }
    }
  ...
  // add instructions to citation linker
  var citationText='<p class="vcul-help-text">Fill in as much of the citation information as you can.</p>';
  ...
  var advancedLink='<a class="EXLSearchFieldRibbonAdvancedTwoLinks" title="Switch to Advanced Search" href="search.do?mode=Advanced&amp;ct=AdvancedSearch&amp;dscnt=0&amp;fromEshelf=false&amp;fromTop=true&amp;fromPreferences=false&amp;dstmp=1377891402235&amp;menuitem=0&amp;vid=VCU" id="advancedSearchBtn">Advanced Search</a>';
  ...
  $('.EXLFacetTile').prepend(closeFacets);
  ...
  var signinLink="<div class='signin-link'><a onclick='ga(\"send\", \"event\", \"link\", \"click\", \"" + ((window.location.hostname == 'vcu-primo.hosted.exlibrisgroup.com') ? 'dev-signin-link' : 'prod-signin-link' )+ "\");' href='http://vcu-primo.hosted.exlibrisgroup.com/primo_library/libweb/action/login.do?loginFn=signin&vid=VCU&targetURL=http://vcu-primo.hosted.exlibrisgroup.com/primo_library/libweb/action/search.do?mode=Basic&vid=VCU&'>Sign in to see all search results</a></div>";
  ...
  var menuDropdown='<a href="#" class="header-dropdown">More search options</a>';
